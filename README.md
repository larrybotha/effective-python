# Effective Python

Notes and annotations from the book [Effective Python]

- [Item 2 - PEP8](./item-02/index.md)
- [Item 3 - `bytes` and `str`](./item-03/index.md)
- [Item 4 - Interpolated strings](./item-04/index.md)
- [Item 5 - Helper Functions Instead Of Complex Expressions](./item-05/index.md)
- [Item 6 - Prefer multiple assignment unpacking over indexing](./item-06/index.md)
- [Item 7 - Prefer `enumerate` over `range`](./item-07/index.md)
- [Item 8 - Use `zip` to process iterators in parallel](./item-08/index.md)
- [Item 9 - Avoid `else` blocks after `for` and `while`](./item-09/index.md)
- [Item 10 - Use assignment expressions](./item-10/index.md)
